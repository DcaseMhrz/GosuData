...
def getTeamData(link,finalteamdata):
	...
	worldrank=worldrank.replace("Unranked","1000")
	earnings=x[7].replace(" ","")
	earnings=earnings.replace(",","")
	earnings=earnings.replace("US","")
	if earnings=="-":
		earnings="0"
...
def getPlayerLink(ramen):
	...
	playerlistslink=[]
	number=len(playerlinks)-10
	
	for i in range(10):
		playerlistslink.append(playerlinks[(number+i)])
...
def getPlayerName(ramen,finalplayernames):
	...
	# for i in range(10):
	# 	finalplayernames.append(newplayers[i])

	
	[finalplayernames.append(x) for x in newplayers if x not in finalplayernames]
	if "TBD" in finalplayernames:
		finalplayernames.remove("TBD")
	# print("Verifying Player Names")
...
def getPlayerData(links,finalplayerdata):
	...
	pearnings=pearnings.replace("US","")
	if pearnings=="-":
		pearnings="0"
	elif pearnings=="":
		pearnings="error"
...
plearn1 = [item.replace("US", "") for item in plearn1]
plearn2 = [item.replace("$", "") for item in plearn2]
plearn2 = [item.replace("US", "") for item in plearn2]
